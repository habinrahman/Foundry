...
/** Localized shell for a single `/careers/[slug]` role — data fetching/notFound stays server-side in `page.tsx`. */
export function RoleDetail({ role }: { role: CareerRole }) {
  ...
  return (
    <article className="mx-auto max-w-[800px] px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
      ...
      <section className="mt-12">
        <h2 className="font-heading text-2xl tracking-tight">
          {t.careers.detail.whatWereLookingFor}
        </h2>
      ...
      <div className="mt-14 flex flex-wrap gap-3 border-t border-[var(--border)] pt-8">
        <Link href={`/apply?role=${role.slug}`}>
          <Button variant="primary" size="lg" className="rounded-full px-7">
            {t.careers.detail.applyForRole}
          </Button>
        </Link>
        <Link href="/careers">
          <Button variant="secondary" size="lg" className="rounded-full px-7">
            {t.careers.detail.viewOtherRoles}
          </Button>
        </Link>
      </div>
    </article>
